...
func main() {
	...
	//configure arguments
	var cfg_path = flag.String("c", "ripkg.ini", "ripkg.ini path")
	var src_root = flag.String("s", "/", "source root path to which ripkg.ini paths are considered relative to")
	var dst_root = flag.String("d", "./ripkg.bundle", "destination directory to wich files are copied to")
	flag.Parse()
  ...
  var cfg_data = string(b)
	...
	for {
		...
		filepath.WalkDir(rp, func(path string, info os.DirEntry, err error) error {
			...
			if info.Type() & os.ModeSymlink == os.ModeSymlink {
				...
				if len(interp) > 0 {
					//spew.Dump(interp)
					var sn = filepath.Join(*src_root, interp)
					append_paths = append(append_paths,sn)
				}
				if len(so_names) > 0 {
					//spew.Dump(so_names)
	...
	//TODO: first copy files then symlinks or file not found could happen
	//copy paths to dest
	for _, s := range paths {
